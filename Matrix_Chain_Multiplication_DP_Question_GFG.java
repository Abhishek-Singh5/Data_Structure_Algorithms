...
(arr[i-1] x arr[i]) for i>=1, find the most efficient way to multiply these matrices together. 
...
class Solution {
    ...
    public static int recursion(int i, int j){
        ...
        for(int k = i; k <= (j-1); k++){
            int currentOption = nums[i-1] * nums[k] * nums[j] + recursion(i, k) + recursion(k + 1, j);
            
            answer = Math.min(answer, currentOption);
        }
